...
## 알아둘 절차 (스택 관련)
- 템플릿 후보 조회: `npm run templates:list`
- 원격 템플릿 적용: `npm run stack:apply -- --preset-git <repo-url> --ref <tag-or-branch>`
- 로컬 템플릿 적용: `npm run stack:apply -- --preset-path <preset-dir>`
- 다른 스택으로 전환: `npm run stack:reset` → 새 프리셋 선택 → `npm run stack:apply` → `npm install`
- 일반 하네스만 쓰고 싶을 때: `activeStack: "none"`으로 설정 (자동 lint/test/build 스킵)
- 새 스택 추가: 본체가 아니라 별도 템플릿 저장소를 만들고 `.harness/stacks/README.md`의 외부 프리셋 계약을 따르기
