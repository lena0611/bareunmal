...
이 문서는 새 스택 하네스를 만들 때의 기준입니다. Vue3, 백엔드 API, 배치, 모바일, 라이브러리 패키지처럼 실제 개발 스택은 달라질 수 있지만, 작성 방식은 같은 구조를 따릅니다.
...
| Vue3, 백엔드 API, 배치처럼 특정 스택에서 반복되는 구조, 명령, 검증 기준 | 스택 하네스 |
...
| 프레임워크 | Vue, React, Nest, Express, Fastify, Spring 등 핵심 프레임워크 |
| 폴더 구조 | 화면, route, service, repository, domain, test가 나뉘는 방식 |
...
```json
{
  ...
  "compatibility": {
    ...
    "incompatible": [
      { "package": "vue", "label": "Vue frontend project" },
      { "package": "react", "label": "React frontend project" }
    ]
  },
  "instructions": [
...
| 문서 | 예시 내용 |
| --- | --- |
| `architecture.md` | route, controller, service, repository, domain model의 책임 경계 |
...
프론트엔드 하네스라면 page, feature, store, route, API adapter, design token, accessibility, browser check 같은 항목이 더 중요할 수 있습니다.
...
- route, controller, service, repository 책임 경계
