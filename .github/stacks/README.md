...
## 활성 프리셋
- `.github/policy-harness/profile.json`의 `activeStack` 값이 결재의 단일 진실 출처입니다.
...
## 신규 스택 추가 가이드
1. `.github/stacks/<new-id>/manifest.json` 작성 (id, framework, designPattern, instructions 목록, policiesFile, checksKey, source).
2. `.github/stacks/<new-id>/instructions/*.md` 추가.
3. `.github/stacks/<new-id>/policies.json` 작성 (id, title, documents, ownedAreas, checks).
4. `.github/stacks/<new-id>/scaffold/` 안에 실제 스캐폴드 파일들(src/, package.merge.json, 빌드 설정 등) 을 넣습니다.
5. 새 `checksKey`가 자동 검사에 필요한 신규 규칙을 요구하면 `scripts/policy-harness.mjs`의 `collectViolations()`에 분기 추가.
6. `.github/stacks/README.md`의 표에 등록.
...
- 프리셋 수가 늘거나 외부 저장소에서 공유해야 하면 `.github/stacks/<id>/scaffold/`를 별도 저장소로 뗼어내고 manifest의 `source.type`을 `tiged`로 전환합니다.
