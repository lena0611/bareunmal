...
2. 기존 목록 화면들을 확인해 공지사항, 주문, 쿠폰 목록은 검색 조건과 페이지 번호를 route query에 저장한다는 반복 패턴을 찾습니다.
3. 회원 목록도 같은 방식으로 수정하고 `npm run harness:check`로 검증합니다.
4. 이 패턴을 `.harness/project/architecture-rules.md`에 "관리자 목록 화면의 검색 조건, 페이지 번호, 정렬 조건은 route query를 기준으로 복원한다"는 로컬 룰 후보로 기록합니다.
5. 같은 문제가 반복되면 `.harness/project/workflow-rules.md`에 목록 화면 변경 시 확인 항목으로 승격하고, 가능하면 테스트로 옮깁니다.
