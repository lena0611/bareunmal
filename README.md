...
| 항목 | 역할 |
| --- | --- |
| `.harness/` | 프로젝트 기준, 검증 기준, 세션 문맥, 스택 프리셋을 담는 본체 |
...
`init`은 설치 직후 자동으로 `harness:doctor`와 `harness:check`를 실행합니다. 사용자는 먼저 생성된 `.harness/session/absorb-report.md`를 보고, 필요한 경우 git hook이나 외부 템플릿을 선택합니다.
